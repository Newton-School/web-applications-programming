async function main() {
    ...
    async function fetchMenu() {
        try {
            const response = await fetch('http://localhost:3000/api/menu'); // Replace with actual API URL
            const menu = await response.json();
            console.log("Today's Menu:");
            menu.forEach(item => {
                console.log(
                    `ID: ${item.id} | Name: ${item.itemName} | Available: ${item.availableQuantity} | Price: $${item.price}`
                );
            });
            return menu;
        } catch (error) {
...
async function checkAvailableQuantity() {
    try {
        ...
        updatedMenu.forEach(item => {
            console.log(
                `ID: ${item.id} | Name: ${item.itemName} | Available: ${item.availableQuantity}`
            );
        });
    } catch (error) {
...
async function displayOrders() {
    try {
        const response = await fetch('http://localhost:3000/api/orders'); // Replace with actual API URL
        const orders = await response.json();
        console.log('List of Orders:');
        orders.forEach(order => {
            console.log(
                `Customer: ${order.customerName} | Item ID: ${order.itemId} | Quantity: ${order.quantity}`
            );
        });
    } catch (error) {
